(() => {
  ...
  const DEFAULT_BUNDLED_CSV_URL = "assets/data/default.csv";
  ...
  async function loadBundledDefaultCsv() {
    el.loadStatus.textContent = `Loading bundled default CSV from ${DEFAULT_BUNDLED_CSV_URL} ...`;
    try {
      const res = await fetch(DEFAULT_BUNDLED_CSV_URL, { cache: "no-store" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const text = await res.text();
      parseAndLoadCsv(text, DEFAULT_BUNDLED_CSV_URL, true);
      status(`Loaded bundled default CSV: ${DEFAULT_BUNDLED_CSV_URL}`);
    } catch (err) {
      el.loadStatus.textContent = "Bundled default CSV not found.";
      status(`Could not load bundled default CSV at '${DEFAULT_BUNDLED_CSV_URL}'.
Add your file there or update DEFAULT_BUNDLED_CSV_URL in assets/js/app.js.
Error: ${String(err)}`);
    }
  }
